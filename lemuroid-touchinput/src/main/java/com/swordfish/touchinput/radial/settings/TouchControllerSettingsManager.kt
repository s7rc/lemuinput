...
class TouchControllerSettingsManager(private val sharedPreferences: SharedPreferences) {
    ...
    @Serializable
    data class ButtonOverride(
        val offsetX: Float = 0f,  // Screen offset in normalized coordinates (-1.0 to 1.0)
        val offsetY: Float = 0f,  // Screen offset in normalized coordinates (-1.0 to 1.0)
        val scale: Float? = null, // Individual scale override (null means use global scale)
    )
    ...
    data class Settings(
        ...
        val buttonOverrides: Map<String, ButtonOverride> = emptyMap(), // Key is button identifier
    )
            ...
            cachedSettings.getOrPut(settingsKey) {
                val currentSettings =
                    sharedPreferences.getString(settingsKey, null)
                        ?.let { 
                            try {
                                Json { ignoreUnknownKeys = true }.decodeFromString(Settings.serializer(), it)
                            } catch (e: Exception) {
                                Timber.e(e, "Failed to decode settings, using defaults")
                                null
                            }
                        }
